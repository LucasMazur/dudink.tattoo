...
export default () => {
    ...
    useEffect(() => {
        Axios.get("http://localhost:3001/api/themes/get").then((response) => {
            setList(response.data)
            setTitle(response.data[0].theme)
        })

        Axios.post("http://localhost:3001/api/images/get", {style: "temática"}).then((response) => {
    ...
    const updateImages = (val) => {
        Axios.post("http://localhost:3001/api/images/get", {style: val}).then((response) => {
            setImage(response.data)
            setImageUrl(response.data[0].url)
        })
    }
